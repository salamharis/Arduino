...
int x = 100;
int del = 1000;//Set del as 5; the value is the degree of fine tuning for the clock
...
void loop()
{
      ...
      for(int digit1=0;digit1<1000;digit1++){
        ...
        if (digitalRead(buttonplus) == HIGH){
          ...
          pickNumber(digit1%10);//Get the value of single digit
         // delay(del);//delay 5ms

          setbeep = EEPROM.read(1);
              if(digit1%setbeep == 0){
                //tone(led,300);
                }
          delay(del);//delay 5ms
